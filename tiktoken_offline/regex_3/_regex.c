...
{
    BOOL(*has_property)
    (RE_LocaleInfo *locale_info, RE_CODE property, Py_UCS4 ch);
    BOOL(*at_boundary)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*at_word_start)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*at_word_end)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*at_default_boundary)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*at_default_word_start)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*at_default_word_end)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*at_grapheme_boundary)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*is_line_sep)
    (Py_UCS4 ch);
    BOOL(*at_line_start)
    ...
    BOOL(*at_line_end)
    (RE_StatePtr state, Py_ssize_t text_pos);
    BOOL(*possible_turkic)
    (RE_LocaleInfo *locale_info, Py_UCS4 ch);
    int (*all_cases)(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *codepoints);
    Py_UCS4 (*simple_case_fold)(RE_LocaleInfo *locale_info, Py_UCS4 ch);
    int (*full_case_fold)(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *folded);
    int (*all_turkic_i)(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *cases);
} RE_EncodingTable;
...
Py_LOCAL_INLINE(BOOL) ascii_has_property_ign(RE_CODE property, Py_UCS4 ch)
{
    ...
    if (property == RE_PROP_GC_LU || property == RE_PROP_GC_LL || property == RE_PROP_GC_LT)
    {
        RE_UINT32 value;

        value = re_get_general_category(ch);

        return value == RE_PROP_LU || value == RE_PROP_LL || value == RE_PROP_LT;
    }
...
static BOOL ascii_has_property_wrapper(RE_LocaleInfo *locale_info, RE_CODE property, Py_UCS4 ch)
{
    return ascii_has_property(property, ch);
}
...
static int ascii_all_cases(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *codepoints)
{
    int count;
 ...
 * first.
 */
static int ascii_all_turkic_i(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *cases)
{
    int count;
...
/* Checks whether a character has a property. */
Py_LOCAL_INLINE(BOOL) locale_has_property(RE_LocaleInfo *locale_info, RE_CODE property, Py_UCS4 ch)
{
    RE_UINT32 value;
    RE_UINT32 v;
...
static BOOL locale_has_property_wrapper(RE_LocaleInfo *locale_info, RE_CODE property, Py_UCS4 ch)
{
    return locale_has_property(locale_info, property, ch);
}
...
/* Gets all the cases of a character. */
static int locale_all_cases(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *codepoints)
{
    int count;
    Py_UCS4 other;
 ...
 * first.
 */
static int locale_all_turkic_i(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *cases)
{
    int count;
    Py_UCS4 other;
...
static BOOL unicode_has_property_wrapper(RE_LocaleInfo *locale_info, RE_CODE property, Py_UCS4 ch)
{
    return unicode_has_property(property, ch);
}
...
static BOOL unicode_at_default_boundary(RE_State *state, Py_ssize_t text_pos)
{
    ...
    /* WB3c */
    if (left_prop == RE_WBREAK_ZWJ && re_get_extended_pictographic(right_char) != 0)
        return FALSE;
    ...
    if (right_pos + 1 < state->text_length)
    {
        ...
        if (IS_AHLETTER(left_prop) && (right_prop == RE_WBREAK_MIDLETTER || IS_MIDNUMLETQ(right_prop)) && IS_AHLETTER(right_right_prop))
            return FALSE;
    }
    ...
    if (left_pos - 1 >= 0)
    {
        ...
        if (IS_AHLETTER(left_left_prop) && (left_prop == RE_WBREAK_MIDLETTER || IS_MIDNUMLETQ(left_prop)) && IS_AHLETTER(right_prop))
            return FALSE;
    }
    ...
    if (right_pos + 1 < state->text_length)
    {
        ...
        if (left_prop == RE_WBREAK_HEBREWLETTER && right_prop == RE_WBREAK_DOUBLEQUOTE && right_right_prop == RE_WBREAK_HEBREWLETTER)
            return FALSE;
    }
    ...
    if (left_pos - 1 >= 0)
    {
        ...
        if (left_left_prop == RE_WBREAK_HEBREWLETTER && left_prop == RE_WBREAK_DOUBLEQUOTE && right_prop == RE_WBREAK_HEBREWLETTER)
            return FALSE;
    }
    ...
    if (left_pos - 1 >= 0)
    {
        ...
        if (left_left_prop == RE_WBREAK_NUMERIC && (left_prop == RE_WBREAK_MIDNUM || IS_MIDNUMLETQ(left_prop)) && right_prop == RE_WBREAK_NUMERIC)
            return FALSE;
    }
    ...
    if (right_pos + 1 < state->text_length)
    {
        ...
        if (left_prop == RE_WBREAK_NUMERIC && (right_prop == RE_WBREAK_MIDNUM || IS_MIDNUMLETQ(right_prop)) && right_right_prop == RE_WBREAK_NUMERIC)
            return FALSE;
    }
    ...
    /* WB13a */
    if ((IS_AHLETTER(left_prop) || left_prop == RE_WBREAK_NUMERIC || left_prop == RE_WBREAK_KATAKANA || left_prop == RE_WBREAK_EXTENDNUMLET) &&
...
static BOOL unicode_at_grapheme_boundary(RE_State *state, Py_ssize_t text_pos)
{
    ...
    /* GB6 */
    if (left_prop == RE_GBREAK_L && (right_prop == RE_GBREAK_L || right_prop == RE_GBREAK_V || right_prop == RE_GBREAK_LV || right_prop == RE_GBREAK_LVT))
        return FALSE;

    /* GB7 */
    if ((left_prop == RE_GBREAK_LV || left_prop == RE_GBREAK_V) && (right_prop == RE_GBREAK_V || right_prop == RE_GBREAK_T))
        return FALSE;

    /* GB8 */
    if ((left_prop == RE_GBREAK_LVT || left_prop == RE_GBREAK_T) && right_prop == RE_GBREAK_T)
        return FALSE;
...
{
    return (0x0A <= ch && ch <= 0x0D) || ch == 0x85 || ch == 0x2028 || ch == 0x2029;
}
...
/* Gets all the cases of a character. */
static int unicode_all_cases(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *codepoints)
{
...
Py_LOCAL_INLINE(void) set_error(int status, PyObject *object)
{
    ...
    case RE_ERROR_INVALID_GROUP_REF:
        if (!error_exception)
            error_exception = get_object("tiktoken_offline.regex_3._regex_core", "error");
...
Py_LOCAL_INLINE(BOOL) ByteStack_push(RE_State *state, ByteStack *stack, BYTE item)
{
...
Py_LOCAL_INLINE(BOOL) ByteStack_pop(RE_State *state, ByteStack *stack, BYTE *item)
{
    if (stack->count < 1)
        return FALSE;
...
Py_LOCAL_INLINE(BOOL) push_int8(RE_State *state, ByteStack *stack, RE_INT8 item)
{
    return ByteStack_push(state, stack, (BYTE)item);
}

/* Pushes a uint8 onto a stack of bytes. */
Py_LOCAL_INLINE(BOOL) push_uint8(RE_State *state, ByteStack *stack, RE_UINT8 item)
{
    return ByteStack_push(state, stack, (BYTE)item);
}
...
Py_LOCAL_INLINE(BOOL) push_ssize(RE_State *state, ByteStack *stack, Py_ssize_t item)
{
...
Py_LOCAL_INLINE(BOOL) push_code(RE_State *state, ByteStack *stack, RE_CODE item)
{
    return ByteStack_push_block(state, stack, (void *)&item, sizeof(item));
}
...
Py_LOCAL_INLINE(BOOL) push_pointer(RE_State *state, ByteStack *stack, void *item)
{
    return ByteStack_push_block(state, stack, (void *)&item, sizeof(item));
}
...
Py_LOCAL_INLINE(BOOL) pop_int8(RE_State *state, ByteStack *stack, RE_INT8 *item)
{
    return ByteStack_pop(state, stack, (BYTE *)item);
}

/* Pops a uint8 off a stack of bytes. */
Py_LOCAL_INLINE(BOOL) pop_uint8(RE_State *state, ByteStack *stack, RE_UINT8 *item)
{
    return ByteStack_pop(state, stack, (BYTE *)item);
}
...
Py_LOCAL_INLINE(BOOL) pop_ssize(RE_State *state, ByteStack *stack, Py_ssize_t *item)
{
...
Py_LOCAL_INLINE(BOOL) pop_code(RE_State *state, ByteStack *stack, RE_CODE *item)
{
    return ByteStack_pop_block(state, stack, (void *)item, sizeof(*item));
}
...
Py_LOCAL_INLINE(BOOL) pop_pointer(RE_State *state, ByteStack *stack, void **item)
{
    return ByteStack_pop_block(state, stack, (void *)item, sizeof(*item));
}
...
Py_LOCAL_INLINE(BOOL) in_range_ign(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, Py_UCS4 lower, Py_UCS4 upper, Py_UCS4 ch)
{
    int count;
...
static BOOL same_char_wrapper(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, Py_UCS4 ch1, Py_UCS4 ch2)
{
    return same_char(ch1, ch2);
}

/* Checks whether 2 characters are the same, ignoring case. */
Py_LOCAL_INLINE(BOOL) same_char_ign(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, Py_UCS4 ch1, Py_UCS4 ch2)
{
    int count;
    Py_UCS4 cases[RE_MAX_CASES];
    int i;
...
/* Wrapper for calling 'same_char' via a pointer. */
static BOOL same_char_ign_wrapper(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, Py_UCS4 ch1, Py_UCS4 ch2)
{
...
/* Checks whether a character is in a range. */
Py_LOCAL_INLINE(BOOL) matches_RANGE(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, RE_Node *node, Py_UCS4 ch)
{
...
Py_LOCAL_INLINE(BOOL) in_set_diff(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, RE_Node *node, Py_UCS4 ch);
Py_LOCAL_INLINE(BOOL) in_set_inter(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, RE_Node *node, Py_UCS4 ch);
Py_LOCAL_INLINE(BOOL) in_set_sym_diff(RE_EncodingTable *encoding,
                                      RE_LocaleInfo *locale_info, RE_Node *node, Py_UCS4 ch);
Py_LOCAL_INLINE(BOOL) in_set_union(RE_EncodingTable *encoding, RE_LocaleInfo *locale_info, RE_Node *node, Py_UCS4 ch);
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_ANY_U_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    RE_EncodingTable *encoding;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_CHARACTER(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    Py_UCS4 ch;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_CHARACTER_IGN(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    Py_UCS4 cases[RE_MAX_CASES];
    int case_count;
    ...
    case 1:
    {
        ...
        while (text_ptr < limit_ptr && any_case(text_ptr[0], case_count, cases) == match)
            ++text_ptr;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_CHARACTER_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    Py_UCS4 ch;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_PROPERTY_IGN(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_PROPERTY_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_RANGE_IGN(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_RANGE_IGN_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_RANGE_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
...
Py_LOCAL_INLINE(Py_ssize_t) match_many_SET_IGN_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL match)
{
    void *text;
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
...
Py_LOCAL_INLINE(size_t) count_one(RE_State *state, RE_Node *node, Py_ssize_t text_pos, size_t max_count, BOOL *is_partial)
{
    size_t count;
    ...
    *is_partial = FALSE;
    ...
    case RE_OP_ANY:
        ...
        count = (size_t)(match_many_ANY(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);

        *is_partial = count == (size_t)(state->text_length - text_pos) && count < max_count && state->partial_side == RE_PARTIAL_RIGHT;
    ...
    case RE_OP_ANY_U:
        ...
        count = (size_t)(match_many_ANY_U(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);
    ...
    case RE_OP_CHARACTER:
        ...
        count = (size_t)(match_many_CHARACTER(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);
    ...
    case RE_OP_PROPERTY:
        ...
        count = (size_t)(match_many_PROPERTY(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);
    ...
    case RE_OP_RANGE:
        ...
        count = (size_t)(match_many_RANGE(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);
    ...
    case RE_OP_RANGE_IGN:
        ...
        count = (size_t)(match_many_RANGE_IGN(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);
    ...
    case RE_OP_SET_UNION:
        ...
        count = (size_t)(match_many_SET(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);
    ...
    case RE_OP_SET_UNION_IGN:
        ...
        count = (size_t)(match_many_SET_IGN(state, node, text_pos, text_pos + (Py_ssize_t)count, TRUE) - text_pos);
...
Py_LOCAL_INLINE(Py_ssize_t) simple_string_search(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL *is_partial)
{
    Py_ssize_t length;
    RE_CODE *values;
    Py_UCS4 check_char;
...
Py_LOCAL_INLINE(Py_ssize_t) simple_string_search_ign(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL *is_partial)
{
    Py_ssize_t length;
    RE_CODE *values;
    RE_EncodingTable *encoding;
        ...
        while (text_ptr > limit_ptr)
        {
            if (any_case(text_ptr[-1], case_count, cases))
            {
                ...
                for (;;)
                {
                    ...
                    if (!same_char_ign(encoding, locale_info, text_ptr[-s_pos - 1], values[length - s_pos - 1]))
                        break;
...
Py_LOCAL_INLINE(Py_ssize_t) simple_string_search_rev(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL *is_partial)
{
    Py_ssize_t length;
    RE_CODE *values;
    Py_UCS4 check_char;
        ...
        while (text_ptr > limit_ptr)
        {
            if (text_ptr[-1] == check_char)
            {
                ...
                for (;;)
                {
                    ...
                    if (!same_char(text_ptr[-s_pos - 1], values[length - s_pos - 1]))
                        break;
...
Py_LOCAL_INLINE(Py_ssize_t) fast_string_search_ign(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit)
{
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
    void *text;
...
Py_LOCAL_INLINE(Py_ssize_t) fast_string_search_rev(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit)
{
    void *text;
    Py_ssize_t length;
    RE_CODE *values;
...
Py_LOCAL_INLINE(BOOL) build_fast_tables(RE_State *state, RE_Node *node, BOOL ignore)
{
    Py_ssize_t length;
    RE_CODE *values;
    Py_ssize_t *bad;
    ...
    Py_ssize_t pos;
    BOOL(*is_same_char)
    (RE_EncodingTable * encoding, RE_LocaleInfo * locale_info, Py_UCS4 ch1, Py_UCS4 ch2);
    ...
    while (pos >= 0)
    {
        ...
        while (i > 0)
        {
            ...
            if (is_same_char(state->encoding, state->locale_info, values[s + i], values[pos + i]))
                /* It still matches. */
                --i;
            else
            {
    ...
    *is_partial = FALSE;
    ...
    if (try_fast && node->string.bad_character_offset)
    {
        ...
        if (found_pos < 0 && state->partial_side == RE_PARTIAL_RIGHT)
             ...
             */
            found_pos = simple_string_search(state, node, limit - (Py_ssize_t)(node->value_count - 1), limit, is_partial);
    }
    else
...
Py_LOCAL_INLINE(Py_ssize_t) string_search_fld_rev(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, Py_ssize_t *new_pos, BOOL *is_partial)
{
    RE_EncodingTable *encoding;
    RE_LocaleInfo *locale_info;
    int (*full_case_fold)(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *folded);
    ...
    *is_partial = FALSE;
    ...
    while (s_pos < length || f_pos < folded_len)
    {
        if (f_pos >= folded_len)
        {
            ...
            folded_len = full_case_fold(locale_info, char_at(text, text_pos - 1), folded);
            f_pos = 0;
        }
    ...
    *is_partial = FALSE;
    ...
    if (try_fast && node->string.bad_character_offset)
    {
        ...
        if (found_pos < 0 && state->partial_side == RE_PARTIAL_RIGHT)
             ...
             * try again, starting close to the end.
             */
            found_pos = simple_string_search_ign(state, node, limit - (Py_ssize_t)(node->value_count - 1), limit, is_partial);
    }
    else
...
/* Performs a string search, backwards, ignoring case. */
Py_LOCAL_INLINE(Py_ssize_t) string_search_ign_rev(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t limit, BOOL try_fast, BOOL *is_partial)
{
    Py_ssize_t found_pos;
    ...
    *is_partial = FALSE;
    ...
    if (try_fast && node->string.bad_character_offset)
    {
        ...
        if (found_pos < 0 && state->partial_side == RE_PARTIAL_LEFT)
             ...
             * try again, starting close to the end.
             */
            found_pos = simple_string_search_ign_rev(state, node, limit + (Py_ssize_t)(node->value_count - 1), limit, is_partial);
    }
    else
    ...
    *is_partial = FALSE;
    ...
    if (try_fast && node->string.bad_character_offset)
    {
        ...
        if (found_pos < 0 && state->partial_side == RE_PARTIAL_LEFT)
             ...
             * try again, starting close to the end.
             */
            found_pos = simple_string_search_rev(state, node, limit + (Py_ssize_t)(node->value_count - 1), limit, is_partial);
    }
    else
...
Py_LOCAL_INLINE(int) try_match_ANY(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
    if (text_pos >= state->text_length)
    {
        if (state->partial_side == RE_PARTIAL_RIGHT)
                                       ...
                                       Py_ssize_t text_pos)
{
    ...
    return bool_as_status(text_pos > state->slice_start &&
                          matches_ANY(state->encoding, node, state->char_at(state->text, text_pos - 1)));
}

/* Checks whether there's any character except a line separator at a position.
 */
Py_LOCAL_INLINE(int) try_match_ANY_U(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
    if (text_pos >= state->text_length)
    {
        if (state->partial_side == RE_PARTIAL_RIGHT)
                                         ...
                                         Py_ssize_t text_pos)
{
    ...
    return bool_as_status(text_pos > state->slice_start &&
                          matches_ANY_U(state->encoding, node, state->char_at(state->text, text_pos - 1)));
}
 ...
 */
Py_LOCAL_INLINE(int) try_match_CHARACTER_IGN_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
    if (text_pos <= 0)
    {
        if (state->partial_side == RE_PARTIAL_LEFT)
...
Py_LOCAL_INLINE(int) try_match_DEFAULT_BOUNDARY(RE_State *state, RE_Node *node,
                                                Py_ssize_t text_pos)
{
    return bool_as_status(state->encoding->at_default_boundary(state, text_pos) == node->match);
}

/* Checks whether a position is at the default end of a word. */
Py_LOCAL_INLINE(int) try_match_DEFAULT_END_OF_WORD(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
...
Py_LOCAL_INLINE(int) try_match_DEFAULT_START_OF_WORD(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
...
Py_LOCAL_INLINE(int) try_match_END_OF_STRING_LINE(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
...
/* Checks whether a position is at the end of the string. */
Py_LOCAL_INLINE(int) try_match_END_OF_STRING_LINE_U(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
    return bool_as_status(text_pos >= state->text_length || text_pos ==
...
/* Checks whether a position is on a grapheme boundary. */
Py_LOCAL_INLINE(int) try_match_GRAPHEME_BOUNDARY(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
...
Py_LOCAL_INLINE(int) try_match_RANGE(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
    if (text_pos >= state->text_length)
    {
        if (state->partial_side == RE_PARTIAL_RIGHT)
...
Py_LOCAL_INLINE(int) try_match_SET(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
    if (text_pos >= state->text_length)
    {
        if (state->partial_side == RE_PARTIAL_RIGHT)
...
Py_LOCAL_INLINE(int) try_match_START_OF_LINE(RE_State *state, RE_Node *node,
                                             Py_ssize_t text_pos)
{
    return bool_as_status(text_pos <= 0 || state->char_at(state->text, text_pos - 1) == '\n');
}
                                          ...
                                          RE_Node *node, Py_ssize_t text_pos, RE_Position *next_position)
{
    ...
    while (s_pos < length)
    {
        if (f_pos >= folded_len)
        {
            ...
            folded_len = full_case_fold(locale_info, char_at(state->text, text_pos), folded);
            f_pos = 0;
        }
 ...
 * backwards.
 */
Py_LOCAL_INLINE(int) try_match_STRING_FLD_REV(RE_State *state, RE_NextNode *next, RE_Node *node, Py_ssize_t text_pos, RE_Position *next_position)
{
    Py_ssize_t length;
    Py_UCS4 (*char_at)(void *text, Py_ssize_t pos);
    RE_EncodingTable *encoding;
    ...
    while (s_pos < length)
    {
        if (f_pos >= folded_len)
        {
            ...
            folded_len = full_case_fold(locale_info, char_at(state->text, text_pos - 1), folded);
            f_pos = 0;
        }
                                          ...
                                          RE_Node *node, Py_ssize_t text_pos, RE_Position *next_position)
{
    ...
    for (s_pos = 0; s_pos < length; s_pos++)
    {
        ...
        if (!same_char_ign(encoding, locale_info, char_at(state->text, text_pos + s_pos), values[s_pos]))
            return RE_ERROR_FAILURE;
    }
 ...
 * backwards.
 */
Py_LOCAL_INLINE(int) try_match_STRING_IGN_REV(RE_State *state, RE_NextNode *next, RE_Node *node, Py_ssize_t text_pos, RE_Position *next_position)
{
    Py_ssize_t length;
    Py_UCS4 (*char_at)(void *text, Py_ssize_t pos);
    RE_EncodingTable *encoding;
    ...
    for (s_pos = 0; s_pos < length; s_pos++)
    {
        ...
        if (!same_char_ign(encoding, locale_info, char_at(state->text, text_pos - s_pos - 1), values[length - s_pos - 1]))
            return RE_ERROR_FAILURE;
    }
 ...
 */
Py_LOCAL_INLINE(int) try_match(RE_State *state, RE_NextNode *next, Py_ssize_t text_pos, RE_Position *next_position)
{
    RE_Node *test;
    int status;
...
Py_LOCAL_INLINE(Py_ssize_t) search_start_BOUNDARY(RE_State *state, RE_Node *node, Py_ssize_t text_pos, BOOL *is_partial)
{
    BOOL(*at_boundary)
    (RE_State * state, Py_ssize_t text_pos);
...
Py_LOCAL_INLINE(Py_ssize_t) search_start_BOUNDARY_rev(RE_State *state, RE_Node *node, Py_ssize_t text_pos, BOOL *is_partial)
{
    BOOL(*at_boundary)
    (RE_State * state, Py_ssize_t text_pos);
...
Py_LOCAL_INLINE(Py_ssize_t) search_start_END_OF_LINE(RE_State *state, RE_Node *node, Py_ssize_t text_pos, BOOL *is_partial)
{
    *is_partial = FALSE;
...
Py_LOCAL_INLINE(Py_ssize_t) search_start_END_OF_WORD(RE_State *state, RE_Node *node, Py_ssize_t text_pos, BOOL *is_partial)
{
    BOOL(*at_word_end)
    (RE_State * state, Py_ssize_t text_pos);
...
Py_LOCAL_INLINE(Py_ssize_t) search_start_STRING_FLD(RE_State *state, RE_Node *node, Py_ssize_t text_pos, Py_ssize_t *new_pos, BOOL *is_partial)
{
    *is_partial = FALSE;
...
/* Searches for a string, ignoring case. */
Py_LOCAL_INLINE(Py_ssize_t) search_start_STRING_IGN(RE_State *state, RE_Node *node, Py_ssize_t text_pos, BOOL *is_partial)
{
    *is_partial = FALSE;
...
Py_LOCAL_INLINE(Py_ssize_t) search_start_STRING_REV(RE_State *state, RE_Node *node, Py_ssize_t text_pos, BOOL *is_partial)
{
    *is_partial = FALSE;
...
Py_LOCAL_INLINE(BOOL) insert_guard_span(RE_State *state, RE_GuardList *guard_list, size_t index)
{
    size_t n;
    ...
    if (n > 0)
        memmove(guard_list->spans + index + 1, guard_list->spans + index, n * sizeof(RE_GuardSpan));
    ++guard_list->count;
...
Py_LOCAL_INLINE(void) delete_guard_span(RE_GuardList *guard_list, size_t index)
{
    ...
    if (n > 0)
        memmove(guard_list->spans + index, guard_list->spans + index + 1, n * sizeof(RE_GuardSpan));
    --guard_list->count;
}
...
Py_LOCAL_INLINE(BOOL) is_guarded(RE_GuardList *guard_list, Py_ssize_t text_pos)
{
    ...
    if (count == 0 || text_pos < spans[0].low || text_pos > spans[count - 1].high)
        return FALSE;
                            ...
                            Py_ssize_t text_pos, BOOL protect)
{
    ...
    if (below >= 0 && text_pos - spans[below].high == 1 && spans[below].protect == protect)
    {
...
Py_LOCAL_INLINE(BOOL) guard_repeat(RE_State *state, size_t index, Py_ssize_t text_pos, RE_STATUS_T guard_type, BOOL protect)
{
    RE_GuardList *guard_list;
...
Py_LOCAL_INLINE(Py_ssize_t) guard_range(RE_State *state, RE_GuardList *guard_list, Py_ssize_t lo_pos, Py_ssize_t hi_pos, BOOL protect)
{
    ...
    if (below >= 0 && lo_pos - spans[below].high == 1 && spans[below].protect == protect)
    {
                                                                ...
                                                                fuzzy_type)
{
    ...
    return fuzzy_counts[fuzzy_type] < values[RE_FUZZY_VAL_MAX_BASE +
                                             fuzzy_type] &&
           error_count < values[RE_FUZZY_VAL_MAX_ERR] && error_count < state->max_errors && cost + values[RE_FUZZY_VAL_COST_BASE + fuzzy_type] <= values[RE_FUZZY_VAL_MAX_COST];
}
                                          ...
                                          size_t *fuzzy_counts)
{
    ...
    return fuzzy_counts[RE_FUZZY_INS] < values[RE_FUZZY_VAL_MAX_INS] &&
           error_count < values[RE_FUZZY_VAL_MAX_ERR] && cost + values[RE_FUZZY_VAL_INS_COST] <= values[RE_FUZZY_VAL_MAX_COST] &&
           error_count < state->max_errors;
}

/* Checks whether the errors are within the fuzzy constraints. */
Py_LOCAL_INLINE(BOOL) fuzzy_within_constraints(size_t *fuzzy_counts, RE_Node *fuzzy_node, size_t max_errors)
{
    RE_CODE *values;
    size_t del_count;
    size_t ins_count;
                                             ...
                                             RE_BestChangesList *best_changes_list)
{
    ...
    if (best_changes_list->count >= best_changes_list->capacity)
    {
        ...
        new_lists = (RE_FuzzyChangesList *)safe_realloc(state,
                                                        best_changes_list->lists, new_capacity * sizeof(RE_FuzzyChangesList));
        if (!new_lists)
            return FALSE;
                                      ...
                                      Py_ssize_t pos)
{
    ...
    case RE_OP_CHARACTER_REV:
        return pos > state->slice_start && matches_CHARACTER(state->encoding,
                                                             state->locale_info, test_node, state->char_at(state->text, pos - 1)) == test_node->match;
    case RE_OP_PROPERTY:
...
Py_LOCAL_INLINE(Py_UCS4) folded_char_at(RE_State *state, Py_ssize_t pos, int folded_pos)
{
    int (*full_case_fold)(RE_LocaleInfo *locale_info, Py_UCS4 ch, Py_UCS4 *folded);
    Py_UCS4 folded[RE_MAX_FOLDED];
...
Py_LOCAL_INLINE(BOOL) fuzzy_ext_match_group_fld(RE_State *state, RE_Node *fuzzy_node, int folded_pos)
{
    RE_Node *test_node;
...
found:
    ...
    if (!record_fuzzy(state, data.fuzzy_type, data.fuzzy_type == RE_FUZZY_DEL ? data.new_text_pos : data.new_text_pos - data.step))
        return RE_ERROR_MEMORY;
...
Py_LOCAL_INLINE(int) retry_fuzzy_match_item(RE_State *state, RE_UINT8 op, BOOL search, RE_Node **node, BOOL advance)
{
    size_t *fuzzy_counts;
    RE_FuzzyData data;
    RE_INT8 step;
...
Py_LOCAL_INLINE(int) retry_fuzzy_insert(RE_State *state, RE_Node **node)
{
    ...
    if (state->text_pos == limit || !insertion_permitted(state, state->fuzzy_node, state->fuzzy_counts) || !fuzzy_ext_match(state, state->fuzzy_node, state->text_pos))
    {
        while (count > 0)
        {
            unrecord_fuzzy(state);
...
Py_LOCAL_INLINE(int) fuzzy_match_group_fld(RE_State *state, BOOL search,
                                           RE_Node *node, int *folded_pos, int folded_len, Py_ssize_t *group_pos, int *gfolded_pos, int gfolded_len, RE_INT8 step)
{
    size_t *fuzzy_counts;
    RE_FuzzyData data;
...
Py_LOCAL_INLINE(int) retry_fuzzy_match_group_fld(RE_State *state, RE_UINT8 op,
                                                 BOOL search, RE_Node **node, int *folded_pos, Py_ssize_t *group_pos, int *gfolded_pos)
{
    size_t *fuzzy_counts;
    RE_FuzzyData data;
    int new_folded_pos;
     ...
     * start searching one character later).
     */
    data.permit_insertion = !search || state->text_pos != state->search_anchor || data.new_folded_pos != data.folded_len;
...
Py_LOCAL_INLINE(int) match_one(RE_State *state, RE_Node *node, Py_ssize_t text_pos)
{
    switch (node->op)
    {
    case RE_OP_ANY:
...
Py_LOCAL_INLINE(BOOL) save_best_match(RE_State *state)
{
    ...
    if (!state->best_match_groups)
    {
        ...
        state->best_match_groups = (RE_GroupData *)safe_alloc(state, group_count * sizeof(RE_GroupData));
        if (!state->best_match_groups)
            return FALSE;

        memset(state->best_match_groups, 0, group_count * sizeof(RE_GroupData));
    ...
    for (g = 0; g < group_count; g++)
    {
        ...
        Py_MEMCPY(best->captures, group->captures, group->count * sizeof(RE_GroupSpan));
    }
...
Py_LOCAL_INLINE(void) restore_best_match(RE_State *state)
{
    ...
    for (g = 0; g < group_count; g++)
    {
        ...
        /* Copy the captures for this group. */
        Py_MEMCPY(group->captures, best->captures, best->count * sizeof(RE_GroupSpan));
    }
}
...
{
    return state->reverse ? state->text_pos == state->slice_start : state->text_pos == state->slice_end;
}
                                                                    ...
                                                                    group_2)
{
    return (group_1->current >= 0 && group_2->current >= 0 &&
            same_span(group_1->captures[group_1->current],
                      group_2->captures[group_2->current])) ||
           (group_1->current < 0 &&
            group_2->current < 0);
}
...
advance:
    ...
    for (;;)
    {
        ...
        case RE_OP_END_GREEDY_REPEAT: /* End of a greedy repeat. */
        {
            ...
            if (changed && state->is_fuzzy && rp_data->count >= node->values[1])
            ...
            try_body = changed && (rp_data->count < node->values[2] || ~node->values[2] == 0) && !is_repeat_guarded(state, index, state->text_pos, RE_STATUS_BODY);
            if (try_body)
            {
                ...
                if (body_status < 0)
                {
                    if (body_status == RE_ERROR_PARTIAL && rp_data->count >= node->values[1] && at_end(state))
        ...
        case RE_OP_GREEDY_REPEAT_ONE: /* Greedy repeat for one character. */
        {
            ...
            for (;;)
            {
                ...
                if (!is_repeat_guarded(state, index, state->text_pos + (Py_ssize_t)count * node->step, RE_STATUS_TAIL))
                {
        ...
        case RE_OP_REF_GROUP_FLD: /* Reference to a capture group, ignoring case. */
        {
            ...
            {
                folded_len = full_case_fold(locale_info, char_at(state->text, state->text_pos), folded);
                gfolded_len = full_case_fold(locale_info, char_at(state->text, string_pos), gfolded);
            }
        ...
        case RE_OP_REF_GROUP_FLD_REV: /* Reference to a capture group, ignoring case. */
        {
            ...
            {
                folded_len = full_case_fold(locale_info, char_at(state->text, state->text_pos - 1), folded);
                gfolded_len = full_case_fold(locale_info, char_at(state->text, string_pos - 1), gfolded);
            }
        ...
        case RE_OP_REF_GROUP_IGN: /* Reference to a capture group, ignoring case. */
        {
            ...
            while (string_pos < span->end)
            {
                ...
                if (state->text_pos < state->slice_end &&
                    same_char_ign(encoding, locale_info, char_at(state->text, state->text_pos), char_at(state->text, string_pos)))
                {
                    ++string_pos;
                    ++state->text_pos;
                }
        ...
        case RE_OP_REF_GROUP_IGN_REV: /* Reference to a capture group, ignoring case. */
        {
            ...
            while (string_pos > span->start)
            {
                ...
                if (state->text_pos > state->slice_start &&
                    same_char_ign(encoding, locale_info, char_at(state->text, state->text_pos - 1), char_at(state->text, string_pos - 1)))
                {
                    --string_pos;
                    --state->text_pos;
                }
        ...
        case RE_OP_SET_UNION_IGN_REV:    /* Set union, ignoring case. */
            ...
            if (state->text_pos > state->slice_start &&
                matches_SET_IGN(encoding, locale_info, node, char_at(state->text, state->text_pos - 1)) == node->match)
            {
                state->text_pos += node->step;
                node = node->next_1.node;
            }
        ...
        case RE_OP_STRING: /* A string. */
        {
            ...
            if ((node->status & RE_STATUS_REQUIRED) && state->text_pos == state->req_pos && string_pos < 0)
        ...
        case RE_OP_STRING_IGN: /* A string, ignoring case. */
        {
            ...
            else
            {
                ...
                while (string_pos < length)
                {
                    ...
                    if (state->text_pos < state->slice_end &&
                        same_char_ign(encoding, locale_info, char_at(state->text, state->text_pos), values[string_pos]))
                    {
                        ++string_pos;
                        ++state->text_pos;
                    }
        ...
        case RE_OP_STRING_IGN_REV: /* A string, ignoring case. */
        {
            ...
            else
            {
                ...
                while (string_pos > 0)
                {
                    ...
                    if (state->text_pos > state->slice_start &&
                        same_char_ign(encoding, locale_info, char_at(state->text, state->text_pos - 1), values[string_pos - 1]))
                    {
                        --string_pos;
                        --state->text_pos;
                    }
...
backtrack:
    for (;;)
    {
        ...
        case RE_OP_END_FUZZY: /* End of fuzzy matching. */
        {
            ...
            if (insertion_permitted(state, inner_node, inner_counts) &&
                total_errors(state->fuzzy_counts) + total_errors(inner_counts) <
                    state->max_errors &&
                fuzzy_ext_match(state, inner_node,
                                state->text_pos))
            {
                RE_INT8 step;
                Py_ssize_t limit;
            ...
            else
            {
                ...
                case RE_OP_CHARACTER_REV:
                {
                    ...
                    for (;;)
                    {
                        ...
                        if (same_char(char_at(state->text, pos - 1), ch) == m && !is_repeat_guarded(state, index, pos,
                                                                                                    RE_STATUS_TAIL))
                        {
                            match = TRUE;
                            break;
                ...
                case RE_OP_STRING_FLD:
                {
                    ...
                    for (;;)
                    {
                        ...
                        found = string_search_fld_rev(state, test, pos + folded_length, limit, &new_pos, &is_partial);
                        if (is_partial)
                            return RE_ERROR_PARTIAL;
                ...
                case RE_OP_STRING_FLD_REV:
                {
                    ...
                    for (;;)
                    {
                        ...
                        found = string_search_fld(state, test, pos - folded_length, limit, &new_pos, &is_partial);
                        if (is_partial)
                            return RE_ERROR_PARTIAL;
                ...
                case RE_OP_STRING_IGN:
                {
                    ...
                    for (;;)
                    {
                        ...
                        found = string_search_ign_rev(state, test, pos + length, limit, FALSE, &is_partial);
                        if (is_partial)
                            return RE_ERROR_PARTIAL;
        ...
        case RE_OP_LAZY_REPEAT_ONE: /* Lazy repeat for one character. */
        {
            ...
            step = node->nonstring.next_2.test->step;
            pos = state->text_pos + (Py_ssize_t)count * step;
            available = step > 0 ? state->slice_end - state->text_pos : state->text_pos - state->slice_start;
            ...
            else
            {
                ...
                case RE_OP_STRING:
                {
                    ...
                    for (;;)
                    {
                        ...
                        found = string_search(state, test, pos + 1, limit + length, TRUE, &is_partial);
                        if (is_partial)
                            return RE_ERROR_PARTIAL;
                ...
                case RE_OP_STRING_IGN:
                {
                    ...
                    for (;;)
                    {
                        ...
                        /* Look for the tail string. */
                        found = string_search_ign(state, test, pos + 1, limit + length, TRUE, &is_partial);
                        if (is_partial)
                            return RE_ERROR_PARTIAL;
                ...
                case RE_OP_STRING_REV:
                {
                    ...
                    for (;;)
                    {
                        ...
                        /* Look for the tail string. */
                        found = string_search_rev(state, test, pos - 1, limit - length, TRUE, &is_partial);
                        if (is_partial)
                            return RE_ERROR_PARTIAL;
                                                                   ...
                                                                   saved_groups)
{
    ...
    if (!saved_groups)
    {
        ...
        memset(saved_groups, 0, pattern->true_group_count * sizeof(RE_GroupData));
    }
    ...
    for (g = 0; g < pattern->true_group_count; g++)
    {
        ...
        Py_MEMCPY(copy->captures, orig->captures, orig->count * sizeof(RE_GroupSpan));
                                                          ...
                                                          saved_groups)
{
    ...
    for (g = 0; g < pattern->true_group_count; g++)
    {
        ...
        Py_MEMCPY(group->captures, saved->captures, saved->count * sizeof(RE_GroupSpan));
                                       ...
                                       Py_ssize_t match_pos, Py_ssize_t text_pos)
{
    ...
    if (best_list->count >= best_list->capacity)
    {
        ...
        new_entries = safe_realloc(state, best_list->entries, new_capacity * sizeof(RE_BestEntry));
        if (!new_entries)
            return FALSE;
...
Py_LOCAL_INLINE(int) do_best_fuzzy_match(RE_State *state, BOOL search)
{
    ...
    state->best_text_pos = state->reverse ? state->slice_start : state->slice_end;
...
Py_LOCAL_INLINE(BOOL) state_init_2(RE_State *state, PatternObject *pattern,
                                   ...
                                   BOOL overlapped, int concurrent, BOOL partial, BOOL use_lock, BOOL visible_captures, BOOL match_all, Py_ssize_t timeout)
{
    ...
    if (pattern->call_ref_info_count > 0)
    {
        ...
        if (!state->group_call_guard_list)
            goto error;
        memset(state->group_call_guard_list, 0, pattern->call_ref_info_count * sizeof(RE_GuardList));
    }
    ...
    if (pattern->true_group_count)
    {
        ...
        else
        {
            ...
            if (!state->groups)
                goto error;
            memset(state->groups, 0, pattern->true_group_count * sizeof(RE_GroupData));
    ...
    if (partial)
        state->partial_side = state->reverse ? RE_PARTIAL_LEFT : RE_PARTIAL_RIGHT;
    else
        state->partial_side = RE_PARTIAL_NONE;
    ...
    if (pattern->repeat_count)
    {
        ...
        else
        {
            ...
            if (!state->repeats)
                goto error;
            memset(state->repeats, 0, pattern->repeat_count * sizeof(RE_RepeatData));
        }
    }
    ...
    if (pattern->fuzzy_count)
    {
        ...
        if (!state->fuzzy_guards)
            goto error;
        memset(state->fuzzy_guards, 0, pattern->fuzzy_count * sizeof(RE_FuzzyGuards));
    }
...
Py_LOCAL_INLINE(BOOL) state_init(RE_State *state, PatternObject *pattern,
                                 PyObject *string, Py_ssize_t start, Py_ssize_t end, BOOL overlapped, int concurrent, BOOL partial, BOOL use_lock, BOOL visible_captures, BOOL match_all, Py_ssize_t timeout)
{
                                          ...
                                          PyObject *def)
{
    ...
    if (index == 0)
        return get_slice(self->substring, self->match_start - self->substring_offset, self->match_end - self->substring_offset);
                                                                    ...
                                                                    index)
{
    ...
    if (index == 0)
    {
        ...
        slice = get_slice(self->substring, self->match_start - self->substring_offset, self->match_end - self->substring_offset);
        if (!slice)
            goto error;
    ...
    for (i = 0; i < group->count; i++)
    {
        slice = get_slice(self->substring, group->captures[i].start - self->substring_offset, group->captures[i].end - self->substring_offset);
        if (!slice)
            goto error;
 ...
 */
Py_LOCAL_INLINE(Py_ssize_t) match_get_group_index(MatchObject *self, PyObject *index, BOOL allow_neg)
{
    Py_ssize_t group;
                                            ...
                                            PyObject *def, BOOL allow_neg)
{
    ...
    if (PyLong_Check(index) || PyUnicode_Check(index) || PyBytes_Check(index))
        return match_get_group_by_index(self, match_get_group_index(self, index, allow_neg), def);
...
static PyObject *match_groups(MatchObject *self, PyObject *args, PyObject *kwargs)
{
    PyObject *result;
    size_t g;
...
static PyObject *match_groupdict(MatchObject *self, PyObject *args, PyObject *kwargs)
{
    PyObject *result;
    PyObject *keys;
    Py_ssize_t g;
 ...
 */
Py_LOCAL_INLINE(PyObject *) get_match_replacement(MatchObject *self, PyObject *item, size_t group_count)
{
    Py_ssize_t index;
    ...
    else if (index >= 1 && (size_t)index <= group_count)
    {
        ...
        if (group->current >= 0)
        {
            ...
            return get_slice(self->substring, span->start - self->substring_offset, span->end - self->substring_offset);
        }
        else
            Py_RETURN_NONE;
    }
...
static PyObject *match_expand(MatchObject *self, PyObject *str_template)
{
    ...
    replacement = call("tiktoken_offline.regex_3.regex", "_compile_replacement_helper",
...
static PyTypeObject Capture_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "_regex.Capture",
    sizeof(MatchObject)};
...
static PyObject *match_expandf(MatchObject *self, PyObject *str_template)
{
    ...
    for (g = 0; g < self->group_count + 1; g++)
        ...
        PyTuple_SetItem(args, (Py_ssize_t)g, make_capture_object(&self, (Py_ssize_t)g));
                                                                         ...
                                                                         slice)
{
    ...
    else
    {
        ...
        {
            /* PyTuple_SetItem borrows the reference. */
            PyTuple_SetItem(result, i, match_get_group_by_index(self, cur, Py_None));
            cur += step;
        }
...
static PyObject *match_fuzzy_changes(PyObject *self_, void *unused)
{
    ...
    count = self->fuzzy_counts[RE_FUZZY_SUB] + self->fuzzy_counts[RE_FUZZY_INS] + self->fuzzy_counts[RE_FUZZY_DEL];
    offset = 0;
...
static PyTypeObject Match_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "_regex.Match",
    sizeof(MatchObject)};
...
Py_LOCAL_INLINE(PyObject *) pattern_new_match(PatternObject *pattern, RE_State *state, int status)
{
                                            ...
                                            PyObject *string, BOOL empty)
{
    ...
    if (string != Py_None && index >= 1 && (size_t)index <= state->pattern->public_group_count && group->current >= 0)
    {
        RE_GroupSpan *span;
...
static PyTypeObject Scanner_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "_regex.Scanner",
...
static PyObject *capture_getitem(CaptureObject *self, PyObject *item)
{
    ...
    return get_slice(match->substring, start - match->substring_offset, end - match->substring_offset);
}
...
static PyTypeObject Splitter_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "_regex.Splitter",
    sizeof(SplitterObject)};

/* Creates a new SplitterObject. */
Py_LOCAL_INLINE(PyObject *) pattern_splitter(PatternObject *pattern, PyObject *args, PyObject *kwargs)
{
...
Py_LOCAL_INLINE(PyObject *) pattern_search_or_match(PatternObject *self,
                                                    PyObject *args, PyObject *kwargs, char *args_desc, BOOL search, BOOL match_all)
{
    Py_ssize_t start;
    Py_ssize_t end;
    int conc;
...
static PyObject *pattern_match(PatternObject *self, PyObject *args, PyObject *kwargs)
{
...
static PyObject *pattern_search(PatternObject *self, PyObject *args, PyObject *kwargs)
{
...
Py_LOCAL_INLINE(BOOL) get_limits(PyObject *pos, PyObject *endpos, Py_ssize_t length, Py_ssize_t *start, Py_ssize_t *end)
{
    Py_ssize_t s;
    Py_ssize_t e;
 ...
 */
Py_LOCAL_INLINE(PyObject *) get_sub_replacement(PyObject *item, PyObject *string, RE_State *state, size_t group_count)
{
    Py_ssize_t index;
...
Py_LOCAL_INLINE(PyObject *) pattern_subx(PatternObject *self, PyObject *str_template, PyObject *string, Py_ssize_t maxsub, int sub_type, PyObject *pos, PyObject *endpos, int concurrent, Py_ssize_t timeout)
{
    RE_StringInfo str_info;
    Py_ssize_t start;
    Py_ssize_t end;
...
static PyObject *pattern_sub(PatternObject *self, PyObject *args, PyObject *kwargs)
{
    int conc;
    Py_ssize_t tim;
...
static PyObject *pattern_subf(PatternObject *self, PyObject *args, PyObject *kwargs)
{
    int conc;
    Py_ssize_t tim;
...
static PyObject *pattern_subn(PatternObject *self, PyObject *args, PyObject *kwargs)
{
    int conc;
    Py_ssize_t tim;
...
static PyObject *pattern_subfn(PatternObject *self, PyObject *args, PyObject *kwargs)
{
    int conc;
    Py_ssize_t tim;
...
static PyObject *pattern_split(PatternObject *self, PyObject *args, PyObject *kwargs)
{
    int conc;
    Py_ssize_t tim;
    RE_State state;
...
static PyObject *pattern_findall(PatternObject *self, PyObject *args, PyObject *kwargs)
{
    Py_ssize_t start;
    Py_ssize_t end;
    int conc;
...
static PyObject *match_repr(PyObject *self_)
{
    ...
    if (!append_string(list, "<tiktoken_offline.regex_3.Match object; span=("))
        goto error;
    ...
    matched_substring = get_slice(self->substring, self->match_start - self->substring_offset, self->match_end - self->substring_offset);
    ...
    if (self->fuzzy_counts[RE_FUZZY_SUB] != 0 ||
        self->fuzzy_counts[RE_FUZZY_INS] != 0 || self->fuzzy_counts[RE_FUZZY_DEL] != 0)
    {
...
static PyObject *pattern_repr(PyObject *self_)
{
    ...
    if (!append_string(list, "tiktoken_offline.regex_3.Regex("))
        goto error;
    ...
    for (i = 0; i < sizeof(flag_names) / sizeof(flag_names[0]); i++)
    {
        if (self->flags & flag_names[i].value)
        {
            ...
            if (!append_string(list, "tiktoken_offline.regex_3."))
                goto error;
...
static PyTypeObject Pattern_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "_regex.Pattern",
...
Py_LOCAL_INLINE(BOOL) record_subpattern_repeats_and_fuzzy_sections(RE_Node *
                                                                       parent_node,
                                                                   size_t offset, size_t repeat_count, RE_Node *node)
{
    while (node)
    {
                                       ...
                                       RE_CODE flags, Py_ssize_t step, size_t value_count)
{
    ...
    if (pattern->node_count >= pattern->node_capacity)
    {
        ...
        new_node_list = (RE_Node **)re_realloc(pattern->node_list, new_capacity * sizeof(RE_Node *));
        if (!new_node_list)
            goto error;
...
Py_LOCAL_INLINE(BOOL) ensure_group(PatternObject *pattern, size_t group)
{
    ...
    if (new_capacity > old_capacity)
    {
        ...
        memset(new_group_info + old_capacity, 0, (new_capacity - old_capacity) * sizeof(RE_GroupInfo));
...
Py_LOCAL_INLINE(BOOL) ensure_call_ref(PatternObject *pattern, size_t call_ref)
{
    ...
    if (new_capacity > old_capacity)
    {
        ...
        memset(new_call_ref_info + old_capacity, 0, (new_capacity - old_capacity) * sizeof(RE_CallRefInfo));
...
Py_LOCAL_INLINE(BOOL) record_call_ref_defined(PatternObject *pattern, size_t call_ref, RE_Node *node)
{
                                    ...
                                    size_t repeat_depth)
{
    ...
    if (new_capacity > old_capacity)
    {
        ...
        memset(new_repeat_info + old_capacity, 0, (new_capacity - old_capacity) * sizeof(RE_RepeatInfo));
...
Py_LOCAL_INLINE(int) build_GROUP(RE_CompileArgs *args)
{
    ...
    start_node = create_node(args->pattern, forward ? RE_OP_START_GROUP : RE_OP_END_GROUP, 0, 0, 3);
    end_node = create_node(args->pattern, forward ? RE_OP_END_GROUP : RE_OP_START_GROUP, 0, 0, 3);
    if (!start_node || !end_node)
        return RE_ERROR_MEMORY;
...
Py_LOCAL_INLINE(int) build_REPEAT(RE_CompileArgs *args)
{
    ...
    else
    {
        ...
        else
        {
            ...
            repeat_node = create_node(args->pattern, greedy ? RE_OP_GREEDY_REPEAT : RE_OP_LAZY_REPEAT, 0, args->forward ? 1 : -1, 4);
            ...
            {
                repeat_node->op = greedy ? RE_OP_GREEDY_REPEAT_ONE : RE_OP_LAZY_REPEAT_ONE;
            ...
            else
            {
                ...
                end_repeat_node = create_node(args->pattern, greedy ? RE_OP_END_GREEDY_REPEAT : RE_OP_END_LAZY_REPEAT, 0,
                                              args->forward ? 1 : -1, 4);
                if (!end_repeat_node)
                    return RE_ERROR_MEMORY;
    ...
    if (!(args->all_atomic && args->code < args->end_code && args->code[0] != RE_OP_END && !args->within_fuzzy))
        args->all_atomic = FALSE;
 ...
 * characters.
 */
Py_LOCAL_INLINE(void) get_required_chars(PyObject *required_chars, RE_CODE **req_chars, size_t *req_length)
{
    Py_ssize_t len;
    RE_CODE *chars;
    Py_ssize_t i;
...
static PyObject *fold_case(PyObject *self_, PyObject *args)
{
    ...
    if (flags & RE_FLAG_FULLCASE)
    {
        ...
        for (i = 0; i < str_info.length; i++)
        {
            ...
            count = full_case_fold(&locale_info, char_at(str_info.characters, i), codepoints);
            for (j = 0; j < count; j++)
...
static PyObject *has_property_value(PyObject *self_, PyObject *args)
{
    ...
    v = unicode_has_property((RE_CODE)property_value, (Py_UCS4)character) ? 1 : 0;
